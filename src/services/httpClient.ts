...
class AxiosClient {
  ...
  private _handleRequest = async (config: InternalAxiosRequestConfig<AxiosRequestConfig>) => {
    const token = window.localStorage.getItem(APP_TOKEN_LOCALKEY) || ''
  ...
  private _handleError = (error: AxiosError<any>) => {
    if (error.response && !error?.config?.url?.includes('/login')) {
